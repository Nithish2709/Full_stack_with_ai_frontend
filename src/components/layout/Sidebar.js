...
const Sidebar = ({ open, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`sidebar fixed top-0 left-0 h-full z-50 flex flex-col
        ...
        <div className="p-5 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 grad-bg rounded-xl flex items-center justify-center shadow-lg shadow-sky-500/30">
              <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
            <div>
              <h1 className="font-bold text-gray-900 dark:text-white text-sm leading-none">ProManage</h1>
          ...
          {navItems.filter(i => !i.adminOnly || user?.role === 'admin').map(item => (
            <NavLink key={item.to} to={item.to} onClick={onClose}
              className={({ isActive }) =>
                ...
                ${isActive
                  ? 'grad-bg text-white shadow-md shadow-sky-500/20'
                  : 'text-gray-600 dark:text-gray-400 hover:bg-sky-50 dark:hover:bg-sky-900/10 hover:text-sky-700 dark:hover:text-sky-300'}`
              }>
